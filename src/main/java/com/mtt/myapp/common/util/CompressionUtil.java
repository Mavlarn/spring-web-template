...
public abstract class CompressionUtil {
	 ...
	 *
	 * @return The {@link java.util.List} of {@link java.io.File}s with the untared content.
	...
	public static List<File> untar(final File inFile, final File outputDir) {
		...
		try {
			...
			throw new CustomRuntimeException("Error while untar file", e);
		} finally {
			IOUtils.closeQuietly(is);
	...
	public static File ungzip(final File inFile, final File outFile) {
		...
		try {
			...
			LOGGER.debug("Trace is : ", e);
			throw new CustomRuntimeException("Error while ungzip file", e);
		} finally {
